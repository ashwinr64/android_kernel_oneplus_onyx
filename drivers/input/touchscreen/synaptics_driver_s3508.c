...
#endif /*CONFIG_MACH_ONYX*///add by jiachenghui for boot time optimize 2015-5-13
...
#ifdef CONFIG_MACH_ONYX //WayneChang, 2015/11/13, Change MENU key to APPSELECT key
...
static void int_touch_s3508(struct synaptics_ts_data *ts,bool insert_flag)
{
	...
	if( valid_event){
		...
		for( i = 0; i < ts->max_num; i++ ){
				...
				#ifdef CONFIG_MACH_ONYX //shankai@bsp.driver , workaround , fixed the bug that report wrong data when more
		...
		ret = i2c_smbus_read_byte_data(ts->client, F1A_0D_DATA00);
			if((ret & 0x01) && !(ts->pre_btn_state & 0x01))//menu
			{
				if( 0 == atomic_read(&is_touch) ){
					#ifdef CONFIG_MACH_ONYX //shankai, 2016/06/07, Change MENU key to APPSELECT key
...
static int	synaptics_input_init(struct synaptics_ts_data *ts)
{
	...
	#ifdef CONFIG_MACH_ONYX //shankai, 2016/6/7, Change MENU key to APPSELECT key
...
#ifdef CONFIG_MACH_ONYX  // shankai@bsp , 2016-6-6,try to  relase all the key when tp resume
